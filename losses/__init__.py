...
def grad(img):
    (_, c, h, w) = img.size()
    kernel = torch.Tensor([[1, 1, 1],[1, -8, 1],[1, 1, 1]]).cuda()
...
def ssim_other(fused_result, input_other):
    ssim_other = ssim(fused_result, input_other)
    return ssim_other

def ssim_color(fused_result, input_color):
    ssim_color = ssim(fused_result, input_color)
    return ssim_color

def ssim_T2(fused_result, input_T2):
    ssim_T2 = ssim(fused_result, input_T2)
    return ssim_T2
...
def RMI_T2(fused_result, input_T2):
    RMI_loss = RMI(input_T2,fused_result)
    return RMI_loss
def RMI_color(fused_result, input_color):
    RMI_loss = RMI(input_color,fused_result)
    return RMI_loss
def RMI_other(fused_result, input_other):
    RMI_loss = RMI(input_other,fused_result)
    return RMI_loss
